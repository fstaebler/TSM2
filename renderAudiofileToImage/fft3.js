//Stolen from http://rosettacode.org/wiki/Fast_Fourier_transform#JavaScript
...
function real(amplitudes){
	var reals = [];
	for(var i = 0; i < amplitudes.length; i++){
		reals[i] = amplitudes[i].re;
	}
	return reals;
}

function absolute(amplitudes){
	var reals = [];
	for(var i = 0; i < amplitudes.length; i++){
		reals[i] = Math.sqrt((amplitudes[i].re * amplitudes[i].re) + (amplitudes[i].im * amplitudes[i].im));
	}
	return reals;
}
